	...
	<body>
		...
		<?php 
    		$db = new Database(); // call database class from 'dbConnection.php' file with new key word declare 
			 $query = "SELECT * FROM  foodreservation"; //sql Command language
			...
			<div class = "titleContainer">
				<div class="title">
					...
					<?php
						...
						echo "<span style='color: mediumblue; text-align: right; margin-left: 45px;'>".$_GET['msg']."</span>";
						} 
					?>
				...
				<!--End Logout Dropdown-->					
					<a href="foodReservationRegistration.php"><input type = "submit" value = "Add" title = "Add Food Reservation Itme"></a><span class = "toltipText">Reserv Food</span>
				</div>
			</div>
            <div style="overflow-x:auto">
				<table class = "foodReservationTable order-table">
				 <thead>
					...
					<?php 
						$i=1;
						while($row = mysqli_fetch_array($read)){ // 'while loop' used for Data retrieve and used fetch_assoc() function
					?>
							<tr id="<?php echo $row['foodReservationId']?>">
								<td> <?php echo $i++ ?> </td>
								<td style='text-align: left'; > <?php echo $row['foodName']; ?> </td>
								<td> <?php echo $row['foodQuantity']; ?> </td>
								<td> <?php echo $row['foodReservationDate']; ?> </td>
								<td> <i style="color: #98363f"; class = "fa fa-trash remove"></i></td>
								<td><a href="updateFoodReservation.php? id=<?php echo urlencode($row['foodReservationId']);?>"><i style='padding: 0px 40px'; class="fa fa-edit"></i></a></td>
							</tr>
						<?php } ?>
						<?php } else { ?>
...
<!--End Section Sezrch Data from table-->	
    ...
    $(".remove").click(function(){
        ...
        {
            $.ajax({
               url: 'deleteRow/deleteFoodReservation.php',
               type: 'GET',
               data: {id: id},
               error: function() {
