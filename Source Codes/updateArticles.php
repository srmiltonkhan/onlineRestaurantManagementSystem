...
<html>
	...
	<body>
		...
		<?php  
			$db =  new Database();
			$query = "SELECT * FROM slider ORDER BY sliderId";  
			...
			<div>
				<table class = "employeeTble">
					<tr>
						...
						<th width="20%";>Picture Title</th>						
						<th width="65%";>Slider Picture</th>						
                   ...
                   <?php if($result){?>
					...
					<?php 
						$i=1;
						foreach($result as $key => $row){ // 'while loop' used for Data retrieve and used fetch_assoc() function
					?>
							<tr>
								<td> <?php echo $row['sliderId']; ?> </td>
								<td style="text-align:left;"> <?php echo $row['PictureTitle']; ?> </td>
								<td><div class="zoom"><?php echo "<img style='width:100px; height: 40px;' src = ".$row['sliderPicture'].">"; ?> </div></td>							
								<td><a href="updateSliderPicture.php? id=<?php echo urlencode($row['sliderId']);?>"><i style="padding: 0px 7px;font-size:25px;" class="fa fa-edit"></i></a></td>
							</tr>
						<?php } ?>
						<?php } else { ?>
