...
<html>
	...
	<?php
	 ...
	 <?php
		$id = $_GET['id']; 
		$db = new Database();
		$query = "SELECT * FROM latestnews WHERE latestNewsId =$id"; //sql Command language
		$getData = $db->select($query)->fetch_assoc();
		
		
		//check validation
		if(isset($_POST['update'])){
			$NewsTitle = mysqli_real_escape_string($db->link, $_POST['NewsTitle']);
			$NewsDate  = mysqli_real_escape_string($db->link, $_POST['NewsDate']);
			$message  = mysqli_real_escape_string($db->link, $_POST['message']);
			
			//check validation
			if($NewsTitle == "" || $NewsDate == "" || $message == "" ){
				$error = "Failed must not be empty";
			}else{
				$updateQuery = "UPDATE `latestnews` SET `NewsTitle`='$NewsTitle',
				`NewsDate`='$NewsDate',`message`='$message' WHERE latestNewsId='$id'";				
				$update = $db->updateLatestNews($updateQuery);				
			}
		}
			...
			<div class = "sub-menu">
				...
				<div class="foodReservationSubContent">
					...
					<div class="foodReservationSubMenu">
					  <form action="updateLatestNews.php?id=<?php echo $id;?>" method="post">
							<table>
								<tr>
									<td>
										<input id="myInput" type="text" name="NewsTitle" value="<?php echo $getData['NewsTitle']?>" class="form-control"> 
									</td>
								</tr>
								<tr>
									<td>
										<div class="input-group date form_datetime col-md-5" data-date="1979-09-16T05:25:07Z" data-date-format="dd MM yyyy - HH:ii p" data-link-field="dtp_input1">
										<input id="myInput" class="form-control" size="16" type="text" readonly name="NewsDate" value="<?php echo $getData['NewsDate']?>" class="form-control">
								...
								<tr>
									<td>	
										<input style= "margin-top: -5px;margin-bottom:10px;" id="myInput" type="text" name="message" value="<?php echo $getData['message']?>" class="form-control" class="form-control"> 
									</td>
								</tr>								
								<tr>
									<td>
